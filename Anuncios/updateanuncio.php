...
if (!empty($_POST)) {
    ...
    if ($valid) {
        ...
        $sql = "UPDATE anuncios set id =?, nombre = ?,fecha = NOW(),contenido = ?
          WHERE id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id, $nom, $cont, $id));
...
<head>
    ...
    <script>
        function enviarcorreos() {
            ...
            xhttp.open("GET", "enviaranuncios.php", true);
            xhttp.send();
        }
    </script>
...
<body>
    ...
    <form class="form-horizontal" action="updateanuncio.php?id=<?php echo $id ?>" method="post" enctype="multipart/form-data">
        <section>
            <table class="registro">
                <tr>
                    <td class="<?php echo !empty($nomError) ? 'error' : ''; ?>">
                        <label>Nombre del Anuncio: </label>
                        <input class="form-control" name="nom" type="text" placeholder="Nombre del Anuncio" value="<?php echo !empty($nombre) ? $nombre : ''; ?>">
                ...
                <tr>
                    <td class="<?php echo !empty($contError) ? 'error' : ''; ?>">
                        ...
                        <textarea class="form-control" name="cont" placeholder="Descripción del Anuncio: (2000 caracteres máximo)"><?php echo !empty($contenido) ? $contenido : ''; ?></textarea>
        ...
        <div class="row justify-content-center">
            ...
            <div class="col-11">
                ...
                <button type="submit" class="calificar-btn">Actualizar Anuncio</button>
