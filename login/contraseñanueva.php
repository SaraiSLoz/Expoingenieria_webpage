...
if ($_POST) {
    ...
    if ($u = $u2) {
        ...
        $sql = "SELECT COUNT(*) AS count_alumnos FROM alumnos WHERE email = :email;
        SELECT COUNT(*) AS count_jueces FROM jueces WHERE email = :email;
        SELECT COUNT(*) AS count_profesores FROM profesores WHERE email = :email;";
        $stmt0 = $pdo->prepare($sql);
        $stmt0->bindParam(':email', $email);
        $stmt0->execute();
        $count_alumnos = $stmt0->fetchColumn();
        $count_jueces = $stmt0->fetchColumn();
        $count_profesores = $stmt0->fetchColumn();

        if ($count_alumnos > 0) {
            $table_name = "alumnos";
        } elseif ($count_jueces > 0) {
            $table_name = "jueces";
        } elseif ($count_profesores > 0) {
        ...
        $sql = "UPDATE $table_name SET password = :pass WHERE email = :email";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':email', $email);
        $stmt->bindParam(':pass', $u);
        $stmt->execute();

        // Verifica si el número de filas afectadas es mayor a cero para saber si la contraseña se actualizó correctamente
        if ($stmt->rowCount() > 0) {
    ...
    <section class="main">
        <form>
            ...
            <table class="olvide_contraseña">
                ...
                <tr>
                    <td><a href="" class="a">Actualizar</a></td>
                </tr>
            </table>
        </form>
    </section>
