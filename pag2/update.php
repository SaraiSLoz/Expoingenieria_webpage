...
if (!empty($_POST)) {
	...
	$extensiones_validas = array('pdf', 'img', 'mp4');
	...
	if ($valid) {
		...
		$sql = "UPDATE proyecto set id_proy =?, id_categoria = ?,id_AreaEstr = ?,
            titulo = ?,profesor = ?,equipo = ?,descripcion =?,poster = ?,
            archivo_poster = ?,archivo_video = ? WHERE id_proy = ?";
		$q = $pdo->prepare($sql);
		($pos == "S") ? $posr = 1 : $posr = 0;
		$q->execute(array($id, $niv, $ar, $nomp, $noml, $equ, $des, $posr, $upload_poster, $upload_video, $id));
		Database::disconnect();
	}
} else {
	$pdo = Database::connect();
