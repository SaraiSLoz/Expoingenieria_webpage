...
if (!empty($_POST)) {
  ...
  if ($valid) {
    ...
    $sql = "INSERT INTO alumnos (matricula,nombre,contraseña,email,id_carrera, codigo_verificacion, tiempo) VALUES (?, ?, ?, ?, ?, ?, NOW())";
    $q = $pdo->prepare($sql);
    $q->execute(array($matricula, $nombre, $contraseña, $email, $id_carrera, $verification_code));
...
<body>
  ...
  <form action="registro_alumno.php" method="post" autocomplete="off">
    <section class="main">
      <h1>Registro de usuario</h1>
      <table class="registro">
        <tr>
          <td><input type="text" name="matricula" placeholder="Matricula..." id="matricula" class="input1" /> </td>
        </tr>
        <tr>
        ...
        <tr>
          <td><select name="id_carrera" class="input3">
              <option value="">Selecciona carrera</option>
              <?php
              $pdo = Database::connect();
              $query = 'SELECT * FROM carrera';
              foreach ($pdo->query($query) as $row) {
                if ($row['id'] == $id_carrera)
